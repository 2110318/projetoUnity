...
public class FloatingPhysicsCSharp : MonoBehaviour {
	...
	void Start () {		//Only executes when the program starts
		xMovement = Random.Range(-.5f,.5f)*2;   //random value between -0.5 and 0.5, causing some movement on the x axis.
		xtorque = Random.Range(-5.0f,5.0f)*2;	//turns the object on the x axis
		ytorque = Random.Range(-5.0f,5.0f)*2;	//turns the object on the y axis
		ztorque = Random.Range(-5.0f,5.0f)*2;	//turns the object on the z axis
